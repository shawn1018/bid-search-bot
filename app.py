...
# --- 2. AI 智慧分析函數 (★ 直接使用 REST API，徹底避開套件報錯) ---
def ai_analyze_tender_with_google_search(tender_name):
    # 檢查金鑰
    api_key = st.secrets.get("GEMINI_API_KEY")
    if not api_key:
        return "❌ 未設定 GEMINI_API_KEY，請檢查 Streamlit Secrets。"
    ...
    # 直接呼叫 Google 的底層 API，繞過所有 Python 套件的語法限制
    url = f"https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-flash:generateContent?key={api_key}"
    
    # 這是最標準、絕不報錯的 Google Search 連動格式
    payload = {
    ...
    try:
        with httpx.Client(timeout=30.0) as client:
            ...
            if resp.status_code != 200:
                return f"❌ Google API 伺服器錯誤: {resp.status_code}\n{resp.text}"
                
            data = resp.json()
            
            # 解析 AI 回答
            candidate = data.get("candidates", [{}])[0]
            text_response = candidate.get("content", {}).get("parts", [{}])[0].get("text", "無法生成摘要。")
            
            # 提取 Google 搜尋來源網址 (Grounding Metadata)
            source_info = ""
            ...
            return text_response + source_info
            
    except Exception as e:
        return f"API 請求時發生錯誤: {e}"
...
if st.session_state.df is not None:
    ...
    st.write("繞過所有套件報錯，直接呼叫 Google API 查出標案背景與預算。")
    selected_tender = st.selectbox("請選擇想分析的標案:", options=df['內容'].tolist())
    
    if st.button("🚀 執行 AI 深度分析 (含全網預算查詢)"):
        with st.spinner('Gemini 2.5 正在調用 Google 搜尋引擎分析中 (約需10秒)...'):
